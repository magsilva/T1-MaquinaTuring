...
class TestTuringMachine:
	...
	def test_new_turing(self):
		...
		assert self.tm.get_decision() == None
	...
	def test_restart(self):
		...
		self.tm.restart()
		initial_configurations = self.tm.get_initial_configurations(tapes)
		self.tm.load_configurations(initial_configurations)
	...
	def test_run_accept(self):
		...
		result = self.tm.run()
		assert result == True
		assert self.tm.get_decision() == True
	...
	def test_run_reject(self):
		...
		result = self.tm.run()
		assert result == True
		assert self.tm.get_decision() == False
